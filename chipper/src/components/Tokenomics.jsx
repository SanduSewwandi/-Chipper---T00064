...
// ADDED IMPORT: Following your requested import location
...
export default function ChipperLanding() {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col font-sans  select-none overflow-x-hidden bg-white">
      
      {/* SECTION 1: MAIN BACKGROUND SCENE (River/Sky Art) */}
      <div 
        className="w-full h-[580vh] md:h-[150vh] flex flex-col  -mt-85 justify-between p-6 md:p-12 relative"
        style={{ 
      ...
      >
        {/* TOP ROW: Contract Address (Top-Right Aligned) */}
        ...
        {/* ─── ADDED: FLOATING GIF ELEMENT ─── */}
        {/* Centers the GIF over your background canvas without disrupting other static assets */}
        <div className="absolute top-1/2 left-75 -translate-x-1/2 -translate-y-30 z-10 pointer-events-none flex justify-center items-center w-full px-4">
          <img 
            src={animatedGif} 
        ...
        {/* ─── NEW OVERLAY IMAGE AT THE SEPARATION LINE ─── */}
        {/* Placed at the absolute bottom-left of this section, translated 50% downwards to ride the boundary line perfectly */}
        ...
        {/* Dynamic Inner Structural Spacer */}
        <div className="flex-grow" />
      </div>

      {/* SECTION 2: SAND BASE FLOOR (Rectangle 7 Art) */}
      <div 
      ...
      >
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center md:items-end gap-8 z-10">
          ...
          {/* Left Side: Circular White Social Buttons */}
          <div className="flex gap-4 items-center justify-center md:justify-start w-full md:w-auto">
            ...
            {/* Dex Chart */}
            <a href="#" className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-105 active:scale-95 transition-transform">
              <svg className="w-6 h-6 text-black fill-none stroke-current stroke-[2.5]" viewBox="0 0 24 24">
          ...
          {/* Right Side Stack: Completely preserves your exact layout alignment */}
          <div className="relative flex flex-col items-center">
            
            {/* ─── NEW IMAGE POSITIONED ABOVE BOX INDEPENDENTLY ─── */}
           <div className="absolute top-0 right-0 -translate-y-[130%] pb-2 z-20 pointer-events-none">
  <img 
    src={infoBoxIcon} 
    alt="Community Header Asset" 
    className="w-30 h-auto sm:w-45 md:w-30 lg:w-50 object-contain"
  />
</div>

            {/* Right Side: Community Text Info Box */}
            {/* UNCHANGED ORIGINAL PROPERTY SCHEME */}
