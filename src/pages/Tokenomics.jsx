  ...
  return (
    <div id="tokenomics" className="w-full flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <style>{`
        ...
        .animate-slide-right {
          animation: slide-right-mobile 4s ease-in-out infinite;
        }
        @media (min-width: 768px) {
          .animate-slide-right {
            animation: slide-right-desktop 4s ease-in-out infinite;
          }
        }
      `}</style>
      ...
      >
        {/* CONTRACT ADDRESS — Mobile: top-center pill | Desktop: right, bottom 55% */}

        {/* Mobile contract address */}
        <div className="md:hidden absolute z-20 top-78 left-1/2 -translate-x-1/2">
          <div className="flex items-center gap-2 bg-white border-[3px] border-black rounded-full px-4 py-2 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] whitespace-nowrap">
        ...
        <div
          className="hidden md:flex absolute right-12 z-20 items-center gap-4 bg-white border-4 border-black rounded-2xl px-5 py-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
          ...
          >
            {copied ? (
              ...
              <svg
                ...
                className="text-black" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"
              >
                <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 00-3.375-3.375h-1.5a1.125 1.125 0 01-1.125-1.125v-1.5A3.375 3.375 0 006.375 7.5H5.25m11.9-3.664A2.251 2.251 0 0015 2.25h-1.5a2.251 2.251 0 00-1.65.686" />
              </svg>
            )}
        ...
        {/* FLOATING GIF (boat)
    Mobile: left and top
    Desktop: left-positioned, original behaviour
*/}
        <img
          src={animatedGif}
          alt="Animated Content"
          className="absolute z-10 pointer-events-none animate-slide-right
            top-[60%] left-[25%]
            md:left-[10px] md:top-1/2"
          style={{
      ...
      >
        {/* ── DESKTOP LAYOUT ── */}
        <div className="hidden md:flex w-full flex-row justify-between items-end gap-8 z-10">
          ...
          {/* Community Box — bottom right */}
          <div className="relative flex flex-col items-center w-auto">
            <div className="absolute top-0 right-0 -translate-y-[140%] pb-2 z-20 pointer-events-none">
              <img src={infoBoxIcon} alt="Community Header Asset" className="w-30 lg:w-50 h-auto object-contain" />
            </div>
            <div className="w-[460px] bg-white border-4 border-black rounded-2xl p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center -mt-[80px]">
              <CommunityText />
              <JoinButton />
            </div>
          </div>
        ...
        {/* ── MOBILE LAYOUT ── */}
        <div className="flex md:hidden w-full flex-col items-center gap-6 z-10">

          {/* Community Box — top, centered */}
          <div className="relative flex flex-col items-center w-full max-w-sm">
            <div className="absolute bottom-[180%] w-50% left-70 -translate-y-[140%] pb-2 z-20 pointer-events-none">
              <img src={infoBoxIcon} alt="Community Header Asset" className="w-28 sm:w-36 h-auto object-contain" />
            </div>
            <div className="w-full bg-white border-4 border-black rounded-2xl p-5 pb-10 mb-10 flex flex-col items-center mt-6">
  <CommunityText />
  <JoinButton />
</div>
          </div>

          {/* Social Icons — below community box, centered */}
...
function SocialTelegram() {
  return (
    <a href="#" className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-full flex items-center justify-center shadow-md hover:scale-105 active:scale-95 transition-transform">
